...
Other independent variables include \textbf{GDP per capita} and \textbf{percentage of university graduates}. The GDP per capita comes from a dataset from Expansion and the percentage of university graduates come from Eurostat.
...
Reg2 <- lm(Turnout ~ Unempl_rate_less25 + GDPpc + PctUniversityGraduates + Region, data = Spain_data)
```

```{r basic6, results='asis', tidy=TRUE, echo=FALSE}
Spain_data2<- Spain_data[!Spain_data$Region == "Ceuta", ]
Spain_data_no_out<- Spain_data2[!Spain_data2$Region == "Melilla", ]
```
...
```{r stargazer1, results='asis', tidy=TRUE, echo=FALSE, header=FALSE}
...
```{r basic4, results='asis', tidy=TRUE, echo=FALSE}
plot(Reg1,which=2)
```

@jimenez2015electoral mentions an interesting fact about the Spanish political system. He analyses the impact of the dual identity of Spanish citizens as a potential influence on voter behavior. Spain has many autonomous regions that have co-official languages and in a stronger or weaker way, affect the feeling of belonging to the country. This can be reflected in voter turnout, especially when we consider general elections for the national parliament. In that sense, we want to check whether this explains the variability of the impact of youth unemployment on turnout, depending on the feeling of attachment. 
