...
geom_line(data=d2015, aes(x=Region_code, y=Unempl_rate_less25, group = 1), color='blue') + ggtitle("Youth Unemployment per region 2004-2015") + 
  ...
  theme(legend.position = "bottom", legend.key = element_rect(size = 5, fill = 'white', colour = 'white'), legend.key.size = unit(2.0, 'lines'), axis.title.y=element_text(margin=margin(10,10,0,10))) + theme_bw() + theme(axis.title.x = element_text(size = 15, vjust=-.2)) +
...
Reg2 <- lm(Turnout ~ Unempl_rate_less25, data = Spain_data)
stargazer(Reg2,
          header=FALSE,
